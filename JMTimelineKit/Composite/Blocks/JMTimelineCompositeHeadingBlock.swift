...
final class JMTimelineCompositeHeadingBlock: UIView, JMTimelineBlock {
    ...
    func configure(repic: JMRepicItem?, repicTint: UIColor?, state: String) {
    ...
    func apply(style: JMTimelineStyle) {
        let style = style.convert(to: JMTimelineCompositeHeadingStyle.self)
...
fileprivate struct Layout {
    ...
    private var statusLabelHeight: CGFloat {
        let leftX = iconSize.width + gap
        let width = bounds.width - leftX
        return statusLabel.calculateHeight(for: width)
    }
}
